...
int a = 1025;
int* int_p = &a; // & means the address of, storing address of a in p
// int_p = &b; //compilation error, strong typed, int ptr cannot point to char data type
int* int_p2 = int_p;
int* int_p2_addr = &int_p;
int** int_p2_ptr = &int_p;
int* b = a;
// int b = *int_p2;
...
struct {
	uint16_t len; // #include <stdint.h>
    char allocated; // whether occupied
    char segment[500];    
} slice;

// printf("size of int %lu \n", sizeof(int)); // sizeof(data_type) 
// printf("0x%04x \n", int_p); // 0x89473984 i.e. a's address
// printf(a); // 1025
printf("%d\n", *int_p); // look at 4 bytes starting at 0x8947xxxxxxxxxx i.e. 1025
printf("%d\n", &a);
printf("%d\n", int_p);
printf("%d\n", int_p2);
printf("%d\n", int_p2_ptr);
...
printf("%d\n", b);
printf("%lu\n", sizeof(slice)); // 501
// printf("size %lu \n", sizeof(int_p2_ptr)); // pointer/ pointer to pointer all 8 bytes
