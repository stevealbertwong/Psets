...
const int LABEL_SIZE = 3; // change label output amount
...
void slda::load_model(const char * filename)
{
    ...
    log_prob_w = new double * [num_topics]; // allocate storage space
    for (int k = 0; k < num_topics; k++)
    {
        log_prob_w[k] = new double [size_vocab];
...
void slda::infer_only(corpus * c, const settings * setting, const char * directory)
{
    ...
    for (d = 0; d < c->num_docs; d++)
    {
        ...
        // label = num_classes-1;
        // base_score = 0.0;
        int label_arr[LABEL_SIZE] = {0}; // initialize to all zero
        double base_score_arr[LABEL_SIZE] = {0.0};
        for (int z = LABEL_SIZE; z > 0; z--) {
            base_score_arr[z-1] = -5.0;
        }
        double swap;
        ...
        for (i = 0; i < num_classes-1; i ++)
        {
            ...
            printf("new score is %f, top elemetn in base_score_arr %f\n", score, base_score_arr[0]);
            
            // comparing label score
            // if score bigger then last element
            if (score > base_score_arr[LABEL_SIZE-1]){
                printf("%lf is bigger than %lf, second last : %lf top element: %lf \n", score, base_score_arr[LABEL_SIZE-1], base_score_arr[LABEL_SIZE-2], base_score_arr[0]);

            	base_score_arr[LABEL_SIZE-1] = score;
            	label_arr[LABEL_SIZE-1] = i;
                // printf("%f\n", base_score_arr[LABEL_SIZE-1]);
                // printf("%d\n", label_arr[LABEL_SIZE-1]);

            	// bubble sort + backward loop
                printf("before sort :\n");
            	for (int h = LABEL_SIZE; h > 0; h--)
                {                    
                    printf(" %f ", base_score_arr[h-1]);
                }
                
                for (int q = LABEL_SIZE-1; q > 0; q--)
                {                    
                                
            		if (base_score_arr[q] > base_score_arr[q-1])
            			{
            				swap = base_score_arr[q];
                            printf("swap: %lf\n", swap);
            				base_score_arr[q] = base_score_arr[q-1];
            				base_score_arr[q-1] = swap;
                            printf("updated: %lf\n", base_score_arr[q-1]);


            				swap = label_arr[q];
                            printf("swap: %lf\n", swap);
            				label_arr[q] = label_arr[q-1];
            				label_arr[q-1] = swap;
                ...
                for (unsigned r = LABEL_SIZE; r > 0; r--){
                    printf(" %lf ", base_score_arr[r-1]);
                }                
            }

            

            // // TODO: return multple label
            // if (score > base_score) // keep replacing 
            // {
            //     base_score = score;
            //     label = i;
            // }
        ...
        // printf("finish loop steven\n");

        if (label_arr[0] == doc->label)
            num_correct ++;
        ...
        // we store from first to end
        for (int e = 0; e <= LABEL_SIZE-1; e++)
        {
        	// printf("%d ", label_arr[e]);
            fprintf(inf_label_file, "%d ", label_arr[e]);
        }
        fprintf(inf_label_file, "\n");
